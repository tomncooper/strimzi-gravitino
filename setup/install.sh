...
install_flink() {
    ...
    local timing_file="${STATUS_DIR}/flink.time"
    local start_time=$(date +%s)
    {
        ...
        local duration=$((end_time - start_time))
        echo "$duration" > "$timing_file"
        echo "[Flink] ✓ Installed successfully (${duration}s)"
        return 0
    } &> "$log_file"
}
...
if [ -f "${STATUS_DIR}/flink" ] && [ "$(cat "${STATUS_DIR}"/flink)" == "SUCCESS" ]; then
    FLINK_TIME=$(cat "${STATUS_DIR}/flink.time" 2>/dev/null || echo "0")
    echo -e "${GREEN}✓ Flink: SUCCESS ($(format_duration $FLINK_TIME))${NC}"
else
